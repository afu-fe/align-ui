...
export default class ClassParser {
  ...
  str: string;
  char: string;
  constructor(ClassNames: string[], extend: any, cache: any, Platform?: { OS: string }) {
    this.ClassNames = ClassNames;
    this.extend = extend || {};
    this.platform = Platform?.OS || 'ios';
    this.cache = cache;
    this.str = '';
    this.char = '';
  }
  parse = () => {
    ...
    this.ClassNames.forEach((item) => {
      ...
      this.str = blackfilter(this.str, this.platform || 'ios');
      if (prefixes && prefixes === this.platform) {
        style = this.parseUtilityItem(this.str, style);
      }
      if (prefixes === null) {
        style = this.parseUtilityItem(this.str, style);
      }
  ...
  parseUtilityItem = (it: string, styles: any) => {
    // 处理单一属性
    let item = it.trim();
    this.char = item[0] ? item[0] : '';
    let style = {};
    //  ----------------------------- v2.1.4 start -----------------------------
      ...
      case `-`:
        {
          ...
          if (match) {
            if (this.consumePeeked(item, '-m') || this.consumePeeked(item, '-p')) {
              const prop = this.consumePeeked(item, '-m') ? `margin` : `padding`;
      ...
      case `p`:
        {
          ...
          if (match) {
            ...
            const value = item.slice(spacingDirection === 'All' ? 2 : 3, item.length);
            spacing(prop, spacingDirection, value, style, this);
          }
        }
        break;
    ...
    if (this.consumePeeked(item, 'border')) {
      const value = item.slice(6, item.length) || '0';
    ...
    if (this.consumePeeked(item, `text-`)) {
      ...
      if (this.consumePeeked(value, 'shadow-')) {
        ...
        if (this.consumePeeked(shadowv, 'offset-')) {
          ...
          textShadowOffset(v, style);
        } else if (this.consumePeeked(shadowv, 'radius-')) {
          const v = shadowv.slice(7, shadowv.length);
          textShadowRadius(v, style);
        } else {
    ...
    if (this.consumePeeked(item, `flex-`)) {
      ...
      } else if (this.consumePeeked(value, `row`) || this.consumePeeked(value, `col`)) {
        flexDirectionWrap('Direction', value, style);
      } else if (this.consumePeeked(value, `wrap`) || this.consumePeeked(value, `nowrap`)) {
    ...
    if (this.consumePeeked(item, `bottom-`) || this.consumePeeked(item, `-bottom-`)) {
      const value = item.slice(7, item.length);
      insetBox('bottom', value, style);
    }
    if (this.consumePeeked(item, `left-`) || this.consumePeeked(item, `-left-`)) {
      const value = item.slice(5, item.length);
      insetBox('left', value, style);
    }
    if (this.consumePeeked(item, `right-`) || this.consumePeeked(item, `-right-`)) {
      const value = item.slice(6, item.length);
      insetBox('right', value, style);
    }
    if (this.consumePeeked(item, `start-`) || this.consumePeeked(item, `-start-`)) {
      const value = item.slice(6, item.length);
      insetBox('start', value, style);
    }
    if (this.consumePeeked(item, `rotate-`) || this.consumePeeked(item, `-rotate-`)) {
      const value = item.slice(7, item.length);
    ...
    if (['textdl-none', 'textdl-unline', 'textdl-line', 'textdl-unlineline'].includes(item)) {
      // 字体偏移
      textDecorationLine(item, style);
    }
    if (['normal-case', 'uppercase', 'lowercase', 'capitalize'].includes(item)) {
  ...
  consumePeeked = (ele: string, str: string) => {
    // 截断字符串
